...
func createOrUpdateCluster(o *Options, w http.ResponseWriter, r *http.Request) {
	...
	//respond status URL
	sendResponse(w, newClusterResponse(r, clusterState))
}
...
func newClusterResponse(r *http.Request, clusterState *cluster.State) *keb.HTTPClusterResponse {
	return &keb.HTTPClusterResponse{
		...
		Status:               string(clusterState.Status.Status),
		StatusURL:            fmt.Sprintf("%s%s/%s/configs/%d/status", r.Host, r.URL.RequestURI(), clusterState.Cluster.Cluster, clusterState.Configuration.Version),
	}
}
...
func sendResponse(w http.ResponseWriter, payload *keb.HTTPClusterResponse) {
	w.Header().Set("content-type", "application/json")
