...
func TestOccupancyRepository(t *testing.T) {
	...
	testCases := []testCase{
		{
			"create occupancy",
			func(t *testing.T, occupRepo Repository) {
				poolID := uuid.NewString()
				occupEntity, err := occupRepo.CreateWorkerPoolOccupancy(poolID, "component1", 50)
		...
		},
		{
			...
			func(t *testing.T, occupRepo Repository) {
				...
				err := occupRepo.UpdateWorkerPoolOccupancy(poolID, 10)
				require.NoError(t, err)
				occupEntity, err := occupRepo.FindWorkerPoolOccupancyByID(poolID)
				require.NoError(t, err)
		...
		},
		{
			...
			func(t *testing.T, occupRepo Repository) {
				componentList, err := occupRepo.GetComponentList()
				require.NoError(t, err)
		...
		},
		{
			...
			func(t *testing.T, occupRepo Repository) {
				...
				err := occupRepo.UpdateWorkerPoolOccupancy(firstPoolID, 40)
				require.NoError(t, err)
				secondPoolID := "4"
				_, err = occupRepo.CreateWorkerPoolOccupancy(secondPoolID, component, 50)
				require.NoError(t, err)
	...
	for _, testCase := range testCases {
		occupRepo := newPersistentRepository(t)
		unitTestSetup(t, occupRepo, occupancies)
		t.Run(testCase.name, newTestFct(testCase, occupRepo))
		testCleanUp(t, occupRepo)
	}
