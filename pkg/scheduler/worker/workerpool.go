...
func (w *Pool) filterProcessableOpsByMaxRetries(ops []*model.OperationEntity) []*model.OperationEntity {
	...
	for _, op := range ops {
		if op.Retries >= int64(w.config.MaxOperationRetries) {
			err := w.reconRepo.UpdateOperationState(op.SchedulingID, op.CorrelationID, model.OperationStateError, fmt.Sprintf("operation exceeds max. operation retries limit (maxOperationRetries:%d)", w.config.MaxOperationRetries))
			if err != nil {
