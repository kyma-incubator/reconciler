...
func (_m *Commands) CreateSecretTLS(ctx *service.ActionContext, ns string, secretName string) (map[string][]byte, error) {
	...
	var r1 error
	if rf, ok := ret.Get(0).(func(*service.ActionContext, string, string) (map[string][]byte, error)); ok {
		return rf(ctx, ns, secretName)
	}
	if rf, ok := ret.Get(0).(func(*service.ActionContext, string, string) map[string][]byte); ok {
		r0 = rf(ctx, ns, secretName)
	} else {
		if ret.Get(0) != nil {
	...
	if rf, ok := ret.Get(1).(func(*service.ActionContext, string, string) error); ok {
		r1 = rf(ctx, ns, secretName)
	} else {
		r1 = ret.Error(1)
	}
