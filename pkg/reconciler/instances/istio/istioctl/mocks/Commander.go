...
func (_m *Commander) Version(istioCtlPath string, kubeconfigPath string) (istioctl.IstioVersion, error) {
	ret := _m.Called(istioCtlPath, kubeconfigPath)

	var r0 istioctl.IstioVersion
	if rf, ok := ret.Get(0).(func(string, string) istioctl.IstioVersion); ok {
		r0 = rf(istioCtlPath, kubeconfigPath)
	} else {
		r0 = ret.Get(0).(istioctl.IstioVersion)
	}
