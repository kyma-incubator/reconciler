...
func Test_Run(t *testing.T) {
	...
	t.Run("should not label pods in kyma-system, kube-system and kyma-integration", func(t *testing.T) {
		// Given
		_, err = client.CoreV1().Namespaces().Create(context.TODO(), &v1.Namespace{ObjectMeta: metav1.ObjectMeta{Name: "kyma-system"}}, metav1.CreateOptions{})
		require.NoError(t, err)
		_, err = client.CoreV1().Namespaces().Create(context.TODO(), &v1.Namespace{ObjectMeta: metav1.ObjectMeta{Name: "kube-system"}}, metav1.CreateOptions{})
		require.NoError(t, err)
		_, err = client.CoreV1().Namespaces().Create(context.TODO(), &v1.Namespace{ObjectMeta: metav1.ObjectMeta{Name: "kyma-integration"}}, metav1.CreateOptions{})
		require.NoError(t, err)

		kymaSystemPod := v1.Pod{ObjectMeta: metav1.ObjectMeta{Namespace: "kyma-system", Name: testPodName, Annotations: map[string]string{"sidecar.istio.io/inject": "false"}}}
		defer func() {
			err = client.CoreV1().Pods("kyma-system").Delete(context.TODO(), kymaSystemPod.Name, metav1.DeleteOptions{})
			require.NoError(t, err)
		}()
		_, err := client.CoreV1().Pods("kyma-system").Create(context.TODO(), &kymaSystemPod, metav1.CreateOptions{})
		require.NoError(t, err)
