...
kyma_get_previous_release_version_return_version=$(git tag -l '*([0-9]).*([0-9]).*([0-9])'| sort -r -V | sed -n 2p)
...
kyma_get_last_release_version_return_version=$(git tag -l '*([0-9]).*([0-9]).*([0-9])'| sort -r -V | head -n1)
